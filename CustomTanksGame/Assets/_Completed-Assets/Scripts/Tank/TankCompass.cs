...
public class TankCompass : MonoBehaviour {
    ...
    private void PointToCheckPoint() {
        ...
        Vector3 to = m_Checkpoints[m_Next].transform.position;
    ...
    private void ResizeArrow() {
        ...
        float distance = (m_Checkpoints[m_Next].transform.position - m_Slider.transform.position).magnitude;
        distance = distance / 2;
    ...
    public void HitCheckpoint(Transform checkpoint)
    {
        if (m_Checkpoints[m_Next] == checkpoint) GetNextCheckpoint();
    }
